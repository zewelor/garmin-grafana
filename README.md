...
> [!TIP]
> The Docker image is published to GHCR as `ghcr.io/zewelor/garmin-grafana:latest` (latest only, no version tags).
> Make sure the package visibility in GitHub Packages is set to `Public`, otherwise anonymous pulls will fail.
...
✅ You can filter which activity types get imported into the database with `ACTIVITY_TYPE_FILTER` (comma-separated list of type keys). Leave empty or unset to import all activity types. Example: `ACTIVITY_TYPE_FILTER="running,cycling,strength_training"`. Known type keys: `running`, `treadmill_running`, `indoor_running`, `cycling`, `indoor_cycling`, `road_biking`, `mountain_biking`, `walking`, `hiking`, `mountaineering`, `strength_training`, `hiit`, `indoor_cardio`, `elliptical`, `lap_swimming`, `open_water_swimming`, `rock_climbing`, `indoor_climbing`, `tennis_v2`, `kayaking_v2`, `boating_v2`, `multi_sport`, `other`.
